...
export default function Advant() {
    ...
    useEffect(() => {
        ...
        axios.get(`${API_BASE_URL}/landing/home/blogs/${pathId}`, { headers: headers, }).then(response => {
                                                ...
                                                <div className="stars">
                                                    {
                                                        Array.from({ length: 5 }).map((_, index) => (
                                                            <i className={`fa-star ${data.rating > index ? 'fa-solid' : 'fa-regular'}`} key={index}></i>
                                    ...
                                    <div className="imgs-cont">
                                        {
                                            data.images.map((item, index) =>
                                                <a data-fancybox="gallery" href={item} className="img-cont" key={index}>
                                                    <Image src={item} alt="Saudi-Arabia" data-caption="Caption #1" width={100} height={100} />
                                                </a>
                                            )
                                        }
                                    </div>
