...
export default function Who() {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get(`${API_BASE_URL}/landing/home/about`, { headers: { lang } })
            .then((res) => {
                if (res.data?.data) {
    ...
    if (error) return <div className="container m-auto py-10 text-center text-red-500">{error}</div>;
    if (!data) return <div className="container m-auto py-10 text-center">No data available</div>;
                            ...
                            <div className="numbers-cont">
                                {data.counters.map((item, index) => (
                                    <motion.div
                                        ...
                                        transition={{ type: 'spring', bounce: 0.5, duration: index * 0.5 }}
                                        viewport={{ once: true }}
                                        className="number"
                                    >
                                        ...
                                        <div className="needed">
                                            <NumberTicker
                                                value={Number(item.counter) > 1000000 ? Number(item.counter) / 1000000 : Number(item.counter)}
                                            />
