...
export default function PackageListSection({
	...
	if (types.length === 0 && packages.length === 0) {
		return (
			<div className="text-center py-12">
				<h3 className="text-xl font-semibold text-gray-600">
					{getTranslation("noPackagesAvailable", lang)}
				</h3>
			</div>
		);
	}
	...
	return (
		<div className={`flex flex-col md:flex-row gap-8${rtl ? " rtl" : ""}`}>
			{/* Filter Sidebar */}
			{types.length > 0 && (
				<aside className="md:w-64 w-full">
					<div className="bg-white rounded-xl shadow p-6 mb-4">
						<h2 className="font-bold text-lg mb-4">
							{getTranslation("types", lang)}
						</h2>
			...
			<section className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-8">
				{filteredPackages.map((pkg) => (
					<PackageCard key={pkg.id} pkg={pkg} types={types} lang={lang} />
				))}
			</section>
		</div>
	);
...
function PackageCard({ pkg, types, lang }) {
	const typeName = types.find((t) => t.id === pkg.type_id)?.name || "";
	const rtl = isRTL(lang);

	return (
		<Card
			className={`overflow-hidden rounded-2xl shadow-lg ${rtl ? " rtl" : ""}`}
		>
			...
			<CardContent className="pt-4">
				...
				>
					<span className="text-xs font-semibold uppercase tracking-wider text-[#ea9623]">
						{typeName}
					</span>
				</div>
				...
				>
					{pkg.places.map((place) => (
						...
						>
							<svg width="16" height="16" fill="#ea9623" viewBox="0 0 24 24">
				...
				>
					...
					>
						...
						{pkg.days} {getTranslation("days", lang)}
					</span>
					{Object.entries(pkg.nights).map(([place, nights]) => (
						...
						>
							...
							{place}:{nights} {getTranslation("nights", lang)}
						</span>
					))}
				</div>
				<div
				...
				>
					<div >
						<span className="block text-xs text-gray-400">
							{getTranslation("startFrom", lang)}
						</span>
						<span className="font-bold text-lg text-gray-900">
							{pkg.price} SAR
						</span>
						<span className="block text-xs text-gray-400">
							{getTranslation("perPerson", lang)}
						</span>
					</div>
					<Link href={pkg.href}>
						<Button
